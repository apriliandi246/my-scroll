class MultiScroll {
  ...
  generateWheelEvent(node) {
    node.addEventListener("wheel", this.wheelEventHandler, {
      once: true,
      passive: true,
    });
  }
  ...
  oneTimeNavigate(direction) {
    ...
    for (let index = 0; index < this.totalSlides; index++) {
      ...
      if (slideType === "full") {
        const fullSlide = slide.firstElementChild;
